...
public:
    constexpr cp_ref(uint16_t index) : index(index) {}
    constexpr const T& get_constant(const class_file& f) const { return f.get_constant<T>(index); }
    constexpr auto get_index() const { return index; }
};

class cp_generic_ref
{
    uint16_t index;

public:
    constexpr cp_generic_ref(uint16_t index) : index(index) {}
    constexpr auto get_index() const { return index; }
    constexpr const cp_info& get_constant(const class_file& f) const { return f.constant_pool[index - 1]; }
};

struct address_reference
{
    int32_t val;
};

struct lvt_reference
{
    uint8_t val;
};
...
struct inst
{
    ...
    uint8_t minor;
    uint8_t inst_sz;
    std::variant<std::monostate, lvt_reference, int, address_reference, cp_generic_ref, cp_ref<fieldref_info>,
                 cp_ref<methodref_info>, cp_ref<class_info>, cp_ref<interface_methodref_info>, cp_ref<invoke_dynamic_info>>
        operand1;
    std::variant<std::monostate, int> operand2;
};
